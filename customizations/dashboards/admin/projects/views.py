...
class GrantPromotionView(forms.ModalFormView):
    ...
    def get_context_data(self, **kwargs):
        return dict(super(
            GrantPromotionView, self).get_context_data(**kwargs).items() +
...
class TransactionHistoryView(tables.DataTableView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super(TransactionHistoryView, self).get_context_data(**kwargs)
        ctx['project'] = api.keystone.tenant_get(self.request,
                                                 ctx['project_id']).name
        return ctx
