...
function run_tests_all {
  ...
  ${command_wrapper} ${coverage_run} $root/manage.py test kili accounting async resource_pricing customizations registration project_billing billing_app keystone_wrapper --settings=kili.test.settings $testopts
  # get results of the kili tests
  KILI_RESULT=$?

  if [ $with_coverage -eq 1 ]; then
    echo "Generating coverage reports"
    check_paths="accounting/*,customizations/*,kili/*,registration/*,async/*,keystone_wrapper/*,resource_pricing/*,billing_app/*,project_billing/*,billing_app/*"
